...
const generateQR = async (req, res) => {
  try {
    ...
    if (!workshop) return sendError(res, 404, 'Workshop not found');
    if (workshop.status !== 'active') return sendError(res, 400, `Workshop is not active. Current status: ${workshop.status}`);

    const { token, expiresAt, timestamp } = generateQRToken(workshop._id.toString(), workshop.qr_seed, type);
    const { qrDataURL, scanURL } = await generateQRImage(token, workshop._id.toString());
...
const scanQR = async (req, res) => {
  try {
    ...
    if (!token || !workshop_id) return sendError(res, 400, 'Token and workshop_id are required');

    const workshop = await Workshop.findById(workshop_id);
    if (!workshop) return sendError(res, 404, 'Workshop not found');
    if (workshop.status !== 'active') return sendError(res, 400, 'Workshop is not currently active');

    const validation = validateQRToken(token, workshop._id.toString(), workshop.qr_seed);
    if (!validation.valid) return sendError(res, 400, validation.reason);

    const scanType = validation.type;
    const now = new Date();

    let log = await AttendanceLog.findOne({ student_id: studentId, workshop_id: workshop._id });

    // ─── Handle ENTRY scan ────────────────────────────────────
    if (scanType === 'entry') {
      if (log && log.entry_time) {
        // Already scanned entry — return friendly success
        return sendSuccess(res, 200, '✅ Entry already recorded!', {
          type: 'entry',
          already_recorded: true,
          verified: false,
          message: 'Your entry is already recorded. Please scan the EXIT QR when leaving!'
        });
      }
      try {
    ...
    if (scanType === 'exit') {
      ...
      if (log.exit_time) {
        // Already scanned exit — return friendly success with verified status
        return sendSuccess(res, 200, '✅ Exit already recorded!', {
          type: 'exit',
          already_recorded: true,
          verified: log.verified_status,
          duration_minutes: log.total_duration_minutes,
          message: log.verified_status
            ? '🎉 Attendance already verified! You can submit feedback and get your certificate.'
            : '⚠️ Exit already recorded but attendance not verified.'
        });
      }
      ...
      if (duration >= workshop.min_duration_minutes && randomCheckPassed) {
    ...
    if (scanType === 'random') {
      ...
      if (log.random_check_passed) {
        return sendSuccess(res, 200, '✅ Random check already completed!', {
          ...
          already_passed: true,
          verified: false,
          message: 'Mid-session check already recorded.'
        });
      }
...
const getAttendanceByWorkshop = async (req, res) => {
  try {
    const logs = await AttendanceLog.find({ workshop_id: req.params.workshopId })
...
const lockAttendance = async (req, res) => {
  try {
    ...
    const logs = await AttendanceLog.find({ workshop_id: workshop._id });
    const verified = logs.filter(l => l.verified_status).length;
...
const exportAttendanceExcel = async (req, res) => {
  try {
    ...
    if (!logs.length) return sendError(res, 404, 'No verified students found for this workshop');

    const { base64, fileName } = await generateAttendanceExcel(workshop, logs);
    return sendSuccess(res, 200, 'Excel generated', { excel_base64: base64, fileName });
  } catch (err) {
