...
import { ArrowLeft, Users, Star, CheckCircle, Clock, BarChart2 } from 'lucide-react';
            ...
            {feedback?.per_question?.length ? (
              <div className="space-y-3">
                {feedback.per_question.map((q, i) => (
                  <RatingBar key={i} label={q.question} value={q.average} />
                ))}
              </div>
            ) : (
            ...
            {feedback?.rating_distribution ? (
              <div className="space-y-2">
                {[5, 4, 3, 2, 1].map((star) => {
                  ...
                  const pct = feedback.total_submissions > 0
                    ? Math.round((count / feedback.total_submissions) * 100)
                    : 0;
                  return (
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              ...
              <div className="space-y-2">
                {feedback.comments.map((c, i) => (
                  <div key={i} className="bg-slate-50 rounded-xl p-3 text-sm text-slate-600 border-l-2 border-indigo-300">
                    {c}
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              ...
              <div className="space-y-2">
                {feedback.suggestions.map((s, i) => (
                  <div key={i} className="bg-slate-50 rounded-xl p-3 text-sm text-slate-600 border-l-2 border-emerald-300">
                    {s}
                  </div>
                ))}
              </div>
