...
import {
  ...
  MessageSquare, Award, LogOut, BookOpen,
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      `}>
        ...
        {/* User info */}
        <div className="p-4 mx-3 mt-3 bg-blue-50 rounded-xl border border-blue-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-blue-800 rounded-full flex items-center justify-center flex-shrink-0">
              <User className="w-4 h-4 text-white" />
            </div>
            <div className="min-w-0">
              <p className="font-semibold text-slate-800 text-sm truncate">{user?.name}</p>
              <p className="text-xs text-slate-500 truncate">{user?.roll_number}</p>
            </div>
          </div>
        </div>
